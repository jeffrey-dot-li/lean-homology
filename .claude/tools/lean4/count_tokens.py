...
Token estimates are rough approximations for comparison purposes.
    ...
    if args.before or args.before_file:
        if not (args.after or args.after_file):
            print(
                "Error: --after or --after-file required for comparison",
                file=sys.stderr,
            )
            return 1
        ...
        if args.before_file:
            ...
            if not match:
                print(
                    f"Error: Invalid format '{args.before_file}'. Use file:start-end",
                    file=sys.stderr,
                )
                return 1
        ...
        if args.after_file:
            ...
            if not match:
                print(
                    f"Error: Invalid format '{args.after_file}'. Use file:start-end",
                    file=sys.stderr,
                )
                return 1
            file_path, start, end = match.groups()
