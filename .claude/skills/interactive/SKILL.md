...
- **Diagnostics before goals.** After every edit, check `lean_diagnostic_messages` (with `severity="error"`) on the edited line(s) first, then `lean_goal`. Never trust `lean_goal` alone — it shows a goal even when the tactic errored.
