...
structure Decomposition (X : Aᵒᵖ ⥤ Type w) {a : A}
  ...
  degree_lt : (GeneralizedReedyCategory.degree) b < (GeneralizedReedyCategory.degree) a
...
def MinusDecomposition.comp {X : Aᵒᵖ ⥤ Type w} {a : A}
  ...
  b := e.b
  σ := d.σ ≫ e.σ
  σ_mem := (GeneralizedReedyCategory.minus).comp_mem d.σ_mem e.σ_mem
  y := e.y
  map_y := by
    calc
...
lemma isDegenerate_iff_exists_minusDecomposition_degree_lt
    ...
    IsDegenerate X x ↔
      ∃ d : MinusDecomposition X x,
        (GeneralizedReedyCategory.degree) d.b < (GeneralizedReedyCategory.degree) a := by
  constructor
  · rintro ⟨d⟩
    obtain ⟨F⟩ :=
      (GeneralizedReedyCategory.factorization).nonempty_mapFactorizationData d.σ
    ...
    have hle : (GeneralizedReedyCategory.degree) F.Z ≤
        (GeneralizedReedyCategory.degree) d.b := by
      by_cases hp : IsIso F.p
      · obtain ⟨h, _⟩ := EilenbergZilberCategory.isIso_eqToHom F.p hp
        exact (congrArg (GeneralizedReedyCategory.degree) h).le
      · exact ((GeneralizedReedyCategory.degree_lt_of_plus) F.p F.hp hp).le
...
lemma degree_le_of_map_eq_nondegenerate
    ...
    (GeneralizedReedyCategory.degree) b ≤ (GeneralizedReedyCategory.degree) c := by
  obtain ⟨F⟩ :=
    (GeneralizedReedyCategory.factorization).nonempty_mapFactorizationData u
  ...
  have hFi : IsIso F.i := by
    ...
    exact ⟨{
      ...
      y := y'
      map_y := hmap
    }, (GeneralizedReedyCategory.degree_lt_of_minus) F.i F.hi h⟩
  obtain ⟨hbZ, _⟩ := EilenbergZilberCategory.isIso_eqToHom F.i hFi
  have hZc : (GeneralizedReedyCategory.degree) F.Z ≤
      (GeneralizedReedyCategory.degree) c := by
    by_cases hFp : IsIso F.p
    · obtain ⟨hZc, _⟩ := EilenbergZilberCategory.isIso_eqToHom F.p hFp
      exact (congrArg (GeneralizedReedyCategory.degree) hZc).le
    · exact ((GeneralizedReedyCategory.degree_lt_of_plus) F.p F.hp hFp).le
...
lemma isIso_of_map_eq_nondegenerate_of_degree_eq
    ...
    (u : b ⟶ c) (y : X.obj (Opposite.op b)) (z : X.obj (Opposite.op c))
    (hu : X.map u.op z = y) (hy : IsNondegenerate X y)
    (hdeg : (GeneralizedReedyCategory.degree) b = (GeneralizedReedyCategory.degree) c) :
    IsIso u := by
  obtain ⟨F⟩ :=
    (GeneralizedReedyCategory.factorization).nonempty_mapFactorizationData u
  let y' := X.map F.p.op z
  ...
  have hFp : IsIso F.p := by
    by_contra h
    have hp := (GeneralizedReedyCategory.degree_lt_of_plus) F.p F.hp h
    have hdegZ : (GeneralizedReedyCategory.degree) F.Z =
        (GeneralizedReedyCategory.degree) b :=
      congrArg (GeneralizedReedyCategory.degree) hbZ.symm
    rw [hdegZ, hdeg] at hp
...
theorem existsUnique_minusDecomposition (X : Aᵒᵖ ⥤ Type w) {a : A}
  ...
  let d₀ : MinusDecomposition X x := {
    b := a
    σ := 𝟙 a
    σ_mem := (GeneralizedReedyCategory.minus).id_mem a
    y := x
    map_y := by simp
  }
  let P : ℕ → Prop := fun m ↦
    ∃ d : MinusDecomposition X x, (GeneralizedReedyCategory.degree) d.b = m
  have hP : ∃ m, P m :=
    ⟨(GeneralizedReedyCategory.degree) a, d₀, rfl⟩
  ...
  have hd_nondegenerate : IsNondegenerate X d.y := by
    ...
    have hmin : Nat.find hP ≤ (GeneralizedReedyCategory.degree) e.b :=
  ...
  have hde : (GeneralizedReedyCategory.degree) d.b ≤
      (GeneralizedReedyCategory.degree) e.b :=
    degree_le_of_map_eq_nondegenerate X u d.y e.y hu hd_nondegenerate
  have hed : (GeneralizedReedyCategory.degree) e.b ≤
      (GeneralizedReedyCategory.degree) d.b :=
    degree_le_of_map_eq_nondegenerate X v e.y d.y hv he_nondegenerate
  have hdegree : (GeneralizedReedyCategory.degree) d.b =
      (GeneralizedReedyCategory.degree) e.b :=
    le_antisymm hde hed
  have hu_iso : IsIso u :=
    isIso_of_map_eq_nondegenerate_of_degree_eq
  ...
  have hdσ_mem : IsMinus (d.σ ≫ eqToHom h) :=
    (GeneralizedReedyCategory.minus).comp_mem d.σ_mem
      ((GeneralizedReedyCategory.isomorphisms_le_minus)
...
def IsInSkeleton (n : ℕ) (X : Aᵒᵖ ⥤ Type w) {a : A}
    (x : X.obj (Opposite.op a)) : Prop :=
  ∃ (b : A) (_ : (GeneralizedReedyCategory.degree) b ≤ n)
...
lemma isInSkeleton_of_degree_le (n : ℕ) (X : Aᵒᵖ ⥤ Type w) {a : A}
    (ha : (GeneralizedReedyCategory.degree) a ≤ n) (x : X.obj (Opposite.op a)) :
...
lemma isDegenerate_of_isInSkeleton (n : ℕ) (X : Aᵒᵖ ⥤ Type w) {a : A}
    (ha : n < (GeneralizedReedyCategory.degree) a) (x : X.obj (Opposite.op a))
...
noncomputable instance costructuredArrowEilenbergZilberCategory
  ...
  refine {
    plus := (GeneralizedReedyCategory.plus).inverseImage π
    minus := (GeneralizedReedyCategory.minus).inverseImage π
    degree := fun Y ↦ (GeneralizedReedyCategory.degree) (π.obj Y)
    isIso_eqToHom := by
      intro U V f hf
      letI : IsIso f := hf
    ...
    isomorphisms_le_plus := by
      intro Y Z f hf
      letI : IsIso f := hf
      exact (GeneralizedReedyCategory.isomorphisms_le_plus) (π.map f) inferInstance
    isomorphisms_le_minus := by
      intro Y Z f hf
      letI : IsIso f := hf
      exact (GeneralizedReedyCategory.isomorphisms_le_minus) (π.map f) inferInstance
    degree_lt_of_plus := by
      intro Y Z f hf hf_noniso
      apply (GeneralizedReedyCategory.degree_lt_of_plus) (π.map f) hf
      intro hπ
      letI : IsIso (π.map f) := hπ
      exact hf_noniso (isIso_of_reflects_iso f π)
    degree_lt_of_minus := by
      intro Y Z f hf hf_noniso
      apply (GeneralizedReedyCategory.degree_lt_of_minus) (π.map f) hf
      intro hπ
      letI : IsIso (π.map f) := hπ
      exact hf_noniso (isIso_of_reflects_iso f π)
    factorization := {
      nonempty_mapFactorizationData := fun f ↦ by
        obtain ⟨F⟩ :=
          (GeneralizedReedyCategory.factorization).nonempty_mapFactorizationData (π.map f)
        exact ⟨MorphismProperty.MapFactorizationData.liftCostructuredArrow
          (GeneralizedReedyCategory.minus).hom (GeneralizedReedyCategory.plus).hom f F⟩
    }
    factorization_unique := by
      intro U V f F G
      ...
      let G₀ := MorphismProperty.MapFactorizationData.projectCostructuredArrow
        (GeneralizedReedyCategory.minus).hom (GeneralizedReedyCategory.plus).hom f G
      obtain ⟨e₀, he₀, he₀_unique⟩ :=
        (GeneralizedReedyCategory.factorization_unique) f.left F₀ G₀
    ...
    degree_eq_of_isIso := by
      ...
      exact (GeneralizedReedyCategory.degree_eq_of_isIso) (π.map f) inferInstance
    iso_eq_id_of_comp_minus := by
      ...
      have hθπ : π.map θ = 𝟙 (π.obj V) :=
        (GeneralizedReedyCategory.iso_eq_id_of_comp_minus) (π.map f) hf (π.map θ)
