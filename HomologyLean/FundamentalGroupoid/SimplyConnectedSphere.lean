...
import HomologyLean.Topology.FoldTrans
...
theorem simply_connected_iff_loops_trivial {Y : Type*} [TopologicalSpace Y] :
  ...
  · intro h _ _ p₁ p₂
    ...
    simp only [fromPath'_trans, fromPath'_symm, fromPath'_refl] at this
    simpa using this
...
attribute [aesop safe (rule_sets := [path_simps])] fromPath'_refl fromPath'_symm fromPath'_trans
...
lemma foldTrans_trans_trans_symm {n : ℕ} (p q : Fin (n + 1) → X)
  ...
  · simp_rw [foldTrans_succ]
    ...
    rw [← fromPath'_eq_iff_homotopic]
    -- aesop_cat?
    -- aesop? (rule_sets := [ path_simps])

    simp_all only [Function.comp_apply, castSucc_zero, castSucc_succ, fromPath'_trans,
    fromPath'_symm, Category.assoc, succ_last, Nat.succ_eq_add_one, IsIso.inv_hom_id_assoc]
    -- simp_all only [Function.comp_apply, castSucc_zero, castSucc_succ, Quotient.mk''_eq_mk, Quotient.mk_trans,
    --   Quotient.mk_symm, Quotient.trans_assoc, succ_last, Nat.succ_eq_add_one]

    -- simp_rw [Fin.castSucc_succ]
    -- simp only [Fin.castSucc_zero, Fin.succ_last, fromPath'_trans, fromPath'_symm]
    -- rw [this]
    -- simp only [Category.assoc, Groupoid.inv_comp, Category.id_comp]
...
-- Provide Fact instances needed for stereographic projections
instance (n : ℕ) : Fact (Module.finrank ℝ (EuclideanSpace ℝ (Fin n)) = n) :=
